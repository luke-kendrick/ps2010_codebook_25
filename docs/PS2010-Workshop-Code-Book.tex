...
\tightlist
\item
  Practice working with ggplot() to produce a bar chart for a data set with two factors and a continuous dependent variable, each with two levels.
\item
  Explore code for a violin plot and a box plot.
\item
  Practice working with ggplot() to produce a line graph for a data set with one factor and a continuous dependent variable.
\item
  Build and interpret a snowman plot.
\end{itemize}
...
Import the \texttt{energy.csv} data set. The code below assumes it will be called \texttt{mydata}.
...
\FunctionTok{scale\_fill\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"redcow"} \OtherTok{=} \StringTok{"slategrey"}\NormalTok{, }\StringTok{"yumster"} \OtherTok{=} \StringTok{"slategray1"}\NormalTok{), }
  ...
  \FunctionTok{scale\_x\_discrete}\NormalTok{(}\AttributeTok{labels =} \FunctionTok{c}\NormalTok{(}\StringTok{"morning"} \OtherTok{=} \StringTok{"Morning"}\NormalTok{, }\StringTok{"afternoon"} \OtherTok{=} \StringTok{"Afternoon"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\item
  \texttt{labels\ =} will allow you to code what the text says within the legend itself. Here we can add capital letters. scale\_x\_discrete`
\item
...
\FunctionTok{scale\_colour\_manual}\NormalTok{(}
  \AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"artificial"} \OtherTok{=} \StringTok{"green2"}\NormalTok{, }\StringTok{"real"} \OtherTok{=} \StringTok{"darkgreen"}\NormalTok{),}
...
Now for some real serious stuff\ldots{}

\texttt{ggplot()} is really powerful and there are so many things you can do with it.

Copy and paste the code below, run it, and interpret the snowman plot\ldots{}

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{install.packages}\NormalTok{(ggforce) }\CommentTok{\# you\textquotesingle{}ll need this package if not installed}
\FunctionTok{library}\NormalTok{(ggforce)}
\FunctionTok{library}\NormalTok{(tidyverse)}
\end{Highlighting}
\end{Shaded}

The code is quite long so expand the section below to copy it.
...
\section{Optional Exercises and Resources}\label{optional-exercises-and-resources}
