...
**Your current data file has the \texttt{total\_sleep} column in. You need to save it so you can use it for next week's workshop. You can overwrite you original .csv file using this code, which will save today's data set.

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{write.csv}\NormalTok{(mydata, }\StringTok{"sleep.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

This means the \texttt{sleep.csv} on your computer will be updated and ready to use next week! Make sure you know where it has saved on your computer before you leave.
...
\item
  ...
  \tightlist
  \item
    Hint: use the code from last week to find the mean and standard deviation for the two groups.
  \end{itemize}
\end{itemize}
...
We will call the object \texttt{int} (short for intervention data)
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{diff }\OtherTok{\textless{}{-}}\NormalTok{ int}\SpecialCharTok{$}\NormalTok{after }\SpecialCharTok{{-}}\NormalTok{ int}\SpecialCharTok{$}\NormalTok{before  }\CommentTok{\#this will calculate the difference score.}
...
(Hint: you will need to use the dollar sign \texttt{\$} to specify which data set andcolumn, e.g., \texttt{data\$column}).
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{longd }\OtherTok{\textless{}{-}}\NormalTok{ int }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{gather}\NormalTok{(time, score, before}\SpecialCharTok{:}\NormalTok{after)}
\end{Highlighting}
\end{Shaded}
...
\item
  Base it on the original data set called \texttt{int} and then \texttt{\%\textgreater{}\%}
\item
  \texttt{gather()}
\item
  The first argument is the name of your independent variable. I have called it \texttt{time} The second argument is the name of your dependent variable. I have called it \texttt{sleep}.
\item
...
\NormalTok{desc }\OtherTok{\textless{}{-}}\NormalTok{ longd }\SpecialCharTok{\%\textgreater{}\%}
  ...
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{mean\_sleep =} \FunctionTok{mean}\NormalTok{(NULL2)}
            \AttributeTok{sd\_sleep =} \FunctionTok{sd}\NormalTok{(NULL2))}
\end{Highlighting}
\end{Shaded}

Change \texttt{NULL1} to the name of the independent variable in the \texttt{long} data file, and \texttt{NULL2} to the dependent variable. If this is tricky, use \texttt{names(long)} if you need a reminder of the variable names and revisit your notes from last week's workshop.
...
\chapter{Workshop 4: One-Way ANOVA (Independent Measures)}\label{workshop-4-one-way-anova-independent-measures}
