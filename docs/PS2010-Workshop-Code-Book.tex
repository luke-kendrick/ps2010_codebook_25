...
\chapter{Workshop 8: Factorial ANOVA: Different Designs and Assumptions}\label{workshop-8-factorial-anova-different-designs-and-assumptions}
...
\begin{quote}
\textbf{Question 10: Which was the only pair that was not significantly different?} -- choose an answer -- A. bike vs.~car B. car vs.~pedestrian C. bike vs.~pedestrian
\end{quote}
...
For the ANOVA model, you should look for partial eta squared (\(η_P^2\)). This is super quick and easy to check from the ANOVA model \texttt{mod} using this code:
...
To check, produce the model output again but this time using \texttt{summary(mod)} as this gives us some extra information.
...
If you look at the degrees of freedom, you'll notice these are a bit unusual as they're not whole numbers. \texttt{aov\_ez()} will always apply the correction, whether we need it or not. How can we check if the assumption was violated?
...
To report the assumption based on the output above, i'd write something like: \textgreater{} Mauchly's test indicated that the assumption of sphericity was not violated for the main effect of hazard, \emph{w} = .96, \emph{p} = .340 or the distraction × hazard interaction, \emph{W} = .96 , \emph{p} = .340.
