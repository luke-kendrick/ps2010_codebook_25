...
\chapter{Workshop 15: Factor Analysis (and Reliability Analysis)}\label{workshop-15-factor-analysis-and-reliability-analysis}
...
\section{Exercise 1: Import and Prepare Data}\label{exercise-1-import-and-prepare-data-1}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Set the working directory {-}WD{-} so R knows where the data lives. Do this by going Session \textgreater{} Set working directory \textgreater{} Choose directory}

\CommentTok{\# You can check the working directory...}

\FunctionTok{getwd}\NormalTok{()}

\CommentTok{\#Before doing anything, need to make sure the right packages are installed and open. We will use all of these......}

\FunctionTok{library}\NormalTok{(tidyverse)}
\FunctionTok{library}\NormalTok{(psych)}

\CommentTok{\#Import the data}
\NormalTok{mydata }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"study\_perception.csv"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\#\# Understanding your data}
\FunctionTok{str}\NormalTok{(mydata) }\CommentTok{\#we can use this to check all of the variables in the file are numeric}
\CommentTok{\#any categorical/factor variables would need to be removed.}

\DocumentationTok{\#\#\# A quick summary of the data}
\FunctionTok{summary}\NormalTok{(mydata)}

\CommentTok{\# Let\textquotesingle{}s create a correlation matrix just to get an understanding of our data file}
\CommentTok{\# we also need to create the correlation matrix to use later}
\NormalTok{corr\_matrix }\OtherTok{\textless{}{-}} \FunctionTok{round}\NormalTok{(}\FunctionTok{cor}\NormalTok{(mydata, }\AttributeTok{use =} \StringTok{"pairwise.complete.obs"}\NormalTok{), }\DecValTok{3}\NormalTok{)}
\FunctionTok{view}\NormalTok{(corr\_matrix)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\#\# Assess the need for factor analysis}
\DocumentationTok{\#\#\# assumptions}
\CommentTok{\# Kaiser{-}Meyer{-}Olkin (KMO) Test {-} Measures Sampling Adequacy}
\NormalTok{kmo\_result }\OtherTok{\textless{}{-}} \FunctionTok{KMO}\NormalTok{(corr\_matrix)}
\FunctionTok{print}\NormalTok{(kmo\_result)  }\CommentTok{\# KMO should be \textgreater{} 0.6 for FA to be appropriate}

\CommentTok{\# Bartlett’s Test of Sphericity {-} Checks if correlation matrix is an identity matrix}
\NormalTok{bartlett\_result }\OtherTok{\textless{}{-}} \FunctionTok{cortest.bartlett}\NormalTok{(corr\_matrix, }\AttributeTok{n =} \FunctionTok{nrow}\NormalTok{(mydata))}
\FunctionTok{print}\NormalTok{(bartlett\_result)  }\CommentTok{\# p{-}value should be \textless{} 0.05 for FA to be suitable}
\end{Highlighting}
\end{Shaded}

\section{Exercise Four}\label{exercise-four}

\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\#\# Determine the number of factors.}
\CommentTok{\# Compute Eigenvalues}
\NormalTok{eigen\_values }\OtherTok{\textless{}{-}} \FunctionTok{eigen}\NormalTok{(corr\_matrix)}\SpecialCharTok{$}\NormalTok{values}
\FunctionTok{print}\NormalTok{(eigen\_values)  }\CommentTok{\# Kaiser’s criterion: Keep factors with eigenvalues \textgreater{} 1}

\CommentTok{\# Scree Plot}
\FunctionTok{fa.parallel}\NormalTok{(mydata, }\AttributeTok{fa =} \StringTok{"fa"}\NormalTok{, }\AttributeTok{n.iter =} \DecValTok{100}\NormalTok{, }\AttributeTok{show.legend =} \ConstantTok{TRUE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\section{Exercise Five}\label{exercise-five}

\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\#\#\# Interpretation of factors.}
\CommentTok{\# Perform Factor Analysis with Varimax Rotation}
\NormalTok{fa\_result }\OtherTok{\textless{}{-}} \FunctionTok{fa}\NormalTok{(mydata, }\AttributeTok{nfactors =} \DecValTok{2}\NormalTok{, }\AttributeTok{rotate =} \StringTok{"varimax"}\NormalTok{, }\AttributeTok{fm =} \StringTok{"ml"}\NormalTok{)  }\CommentTok{\# Adjust nfactors}
\CommentTok{\# Print Factor Loadings (Rotated Component Matrix)}
\FunctionTok{print}\NormalTok{(fa\_result}\SpecialCharTok{$}\NormalTok{loadings, }\AttributeTok{cutoff =} \FloatTok{0.3}\NormalTok{)  }\CommentTok{\# Show only loadings \textgreater{} 0.3}

\CommentTok{\# Visualizing Loadings}
\FunctionTok{fa.diagram}\NormalTok{(fa\_result)  }\CommentTok{\# Shows factor structure in a diagram}
\end{Highlighting}
\end{Shaded}

\section{Exercise Six}\label{exercise-six}

\begin{Shaded}
\begin{Highlighting}[]
\DocumentationTok{\#\#\#\#  Reliability Analysis}
\CommentTok{\# select items and put them onto a scale.}
\CommentTok{\# Group items based on factor loadings}
\FunctionTok{colnames}\NormalTok{(mydata)}
\NormalTok{scale\_1 }\OtherTok{\textless{}{-}}\NormalTok{ mydata }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{select}\NormalTok{(}\FunctionTok{starts\_with}\NormalTok{(}\StringTok{"s1\_"}\NormalTok{))}
\NormalTok{scale\_2  }\OtherTok{\textless{}{-}}\NormalTok{ mydata }\SpecialCharTok{\%\textgreater{}\%} \FunctionTok{select}\NormalTok{(}\FunctionTok{starts\_with}\NormalTok{(}\StringTok{"s2\_"}\NormalTok{))}

\CommentTok{\# Compute Cronbach\textquotesingle{}s Alpha}
\FunctionTok{alpha}\NormalTok{(scale\_1)  }\CommentTok{\# For Factor 1}
\FunctionTok{alpha}\NormalTok{(scale\_2)  }\CommentTok{\# For Factor 2}
\end{Highlighting}
\end{Shaded}
...
\chapter{Workshop 16: Correlations}\label{workshop-16-correlations}

\section{Exercise One}\label{exercise-one}

\textbf{Packages and Import Data}
...
\CommentTok{\# Remember you can use install.packages("NULL") to install.}
\end{Highlighting}
\end{Shaded}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Now get R to open our dataset}

\NormalTok{mydata }\OtherTok{\textless{}{-}} \FunctionTok{read\_csv}\NormalTok{(}\StringTok{"WS\_data\_R\_optimism.csv"}\NormalTok{)}

\CommentTok{\# To check the data have opened ok, you can view the data...}

\FunctionTok{view}\NormalTok{(mydata)}
\end{Highlighting}
\end{Shaded}

\section{Exercise Two}\label{exercise-two-1}

\textbf{Prepare Data}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Next, we need to tell R which variables are continuous (as.numeric) and which are categorical (as.factor).}

\CommentTok{\# You can check the names of the variables with this...}

\FunctionTok{names}\NormalTok{(mydata)}

\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{p\_num }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{p\_num)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{optimism }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{optimism)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{positive\_SC }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{positive\_SC)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{negative\_SC }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{negative\_SC)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{age\_years }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{age\_years)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{extra\_curr }\OtherTok{\textless{}{-}} \FunctionTok{as.factor}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{extra\_curr)}
\NormalTok{mydata}\SpecialCharTok{$}\NormalTok{reading\_age }\OtherTok{\textless{}{-}} \FunctionTok{as.numeric}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{reading\_age)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Before getting into correlations, we might want a summary of our variables. For the continuous variables, we get descriptives. For the categorical/binary variables, we get frequencies.}

\FunctionTok{summary}\NormalTok{(mydata)}

\CommentTok{\# If we want to see the descriptives split for different groups, for example, we want to see the descriptives for optimism for extracurricular activities status separately..}

\NormalTok{descriptives\_bygroup }\OtherTok{\textless{}{-}}\NormalTok{ mydata }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# Tell R which data set to use.  \%\textgreater{}\% means "and then" so tells R to move on and do something else}
  \FunctionTok{group\_by}\NormalTok{(extra\_curr) }\SpecialCharTok{\%\textgreater{}\%} \CommentTok{\# group\_by is telling R to split the data file {-} put the variable to split by in brackets}
  \FunctionTok{summarise}\NormalTok{(}\AttributeTok{mean\_optimism =} \FunctionTok{mean}\NormalTok{(optimism), }\AttributeTok{sd\_optimism =} \FunctionTok{sd}\NormalTok{(optimism)) }\CommentTok{\# Ask for the mean and standard deviation. statistic\_calculated = statistic(variable)}

\CommentTok{\# You then need R to "print" {-} or display {-} the calculated descriptives in the console window.}

\FunctionTok{print}\NormalTok{(descriptives\_bygroup)}
\end{Highlighting}
\end{Shaded}

\section{Exercise Three}\label{exercise-three-1}

\textbf{Run and interpret Pearson's correlations (zero order correlations)}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Now we can look at the correlations between these four continuous variables {-} but we want to mainly focus on the correlations with optimism {-} our main variable of interest.}

\NormalTok{mydata }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(optimism, positive\_SC, negative\_SC, age\_years) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{correlation}\NormalTok{(}\AttributeTok{p\_adjust =} \StringTok{"none"}\NormalTok{)}

\CommentTok{\# In addition to giving you the r and p values, it gives the N, so check that this is correct. To write up the correlation, remember that df = N{-}2.}
\end{Highlighting}
\end{Shaded}

\section{Exercise Four}\label{exercise-four-1}

\textbf{Creating scatterplots with lines of best fit}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# First, let\textquotesingle{}s graph the correlations between "optimism" with life, and the three other continuous variables. You won\textquotesingle{}t see them until after you make them and then ask R to display them. We will make four scatterplots...}

\CommentTok{\# Optimism and positive self compassion}

\NormalTok{plot1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ positive\_SC, }\AttributeTok{y =}\NormalTok{ optimism)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
              \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}

\CommentTok{\# Optimism and negative self compassion}

\NormalTok{plot2 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ negative\_SC, }\AttributeTok{y =}\NormalTok{ optimism)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
              \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}

\CommentTok{\# Optimism and age in years}

\NormalTok{plot3 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ age\_years, }\AttributeTok{y =}\NormalTok{ optimism)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
              \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}

\CommentTok{\# To see what the plots look like, we need to arrange them in the "Plot" window. Make sure "gridextra" is ticked in the "Packages" window}

\FunctionTok{grid.arrange}\NormalTok{(plot1, plot2, plot3, }\AttributeTok{ncol =} \DecValTok{3}\NormalTok{)  }

\CommentTok{\# ncol = 3 tells R to put three next to each other}
\end{Highlighting}
\end{Shaded}

\section{Exercise Five}\label{exercise-five-1}

\textbf{Run and interpret partial correlations}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Next, let\textquotesingle{}s look at partial correlations, so the three main correlations of interest we just ran, but now controlling for reading age.}

\CommentTok{\# You need to have one set of code for each partial correlation, and make sure "ppcor" is ticked in the "Packages" window.}

\FunctionTok{pcor.test}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{optimism, mydata}\SpecialCharTok{$}\NormalTok{positive\_SC,}
\NormalTok{          mydata}\SpecialCharTok{$}\NormalTok{reading\_age,}
          \AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}

\FunctionTok{pcor.test}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{optimism, mydata}\SpecialCharTok{$}\NormalTok{negative\_SC,}
\NormalTok{          mydata}\SpecialCharTok{$}\NormalTok{reading\_age,}
          \AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}

\FunctionTok{pcor.test}\NormalTok{(mydata}\SpecialCharTok{$}\NormalTok{optimism, mydata}\SpecialCharTok{$}\NormalTok{age\_years,}
\NormalTok{          mydata}\SpecialCharTok{$}\NormalTok{reading\_age,}
          \AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\section{Exercise Six}\label{exercise-six-1}

\textbf{Statistically comparing correlations}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Final thing is comparing correlations across different groups. For example, is the correlation between satisfaction with life and negative life experiences different when comparing people who are single or in a relationship?}

\CommentTok{\# First, we need to tell R which subgroups within our dataset we want to look at {-} so identify 0 and 1 from the "relationship" variable, and name each one.}

\NormalTok{None }\OtherTok{\textless{}{-}}\NormalTok{ mydata[mydata}\SpecialCharTok{$}\NormalTok{extra\_curr }\SpecialCharTok{==} \StringTok{"0"}\NormalTok{, ]}
\NormalTok{Activities }\OtherTok{\textless{}{-}}\NormalTok{ mydata[mydata}\SpecialCharTok{$}\NormalTok{extra\_curr }\SpecialCharTok{==} \StringTok{"1"}\NormalTok{, ]}

\CommentTok{\# Now we run the two correlations {-} we need to tell R first which subgroup to use from the naming we just did, and which continuous variable to correlate.}

\FunctionTok{cor.test}\NormalTok{(None}\SpecialCharTok{$}\NormalTok{optimism, None}\SpecialCharTok{$}\NormalTok{positive\_SC,}
         \AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}
\FunctionTok{cor.test}\NormalTok{(Activities}\SpecialCharTok{$}\NormalTok{optimism, Activities}\SpecialCharTok{$}\NormalTok{positive\_SC,}
         \AttributeTok{method =} \StringTok{"pearson"}\NormalTok{)}

\CommentTok{\# To compare the correlations statistically, we need the N and the r for each group. The r value is the final value in the output we just created {-} the final line, under corr.}
\CommentTok{\# To get the N for each group, we ran the "summary" earlier, but you can do it again to save scrolling. }

\FunctionTok{summary}\NormalTok{(mydata)}

\CommentTok{\# Now we can statistically compare our r values. Make a note of which group you consider to be "1" and which is "2". For this, it will be 1 is single and 2 is in a relationship.}
\CommentTok{\# First, make sure "cocor" is ticked in the "Packages" tab.}
\CommentTok{\# r1 is the first r{-}value, in this case 0.2009678}
\CommentTok{\# r2 is the second r{-}value, in this case 0.6345603}
\CommentTok{\# n1 is the first sample size, in this case 73}
\CommentTok{\# n2 is the second sample size, in this case 77}
\CommentTok{\# the code looks like this, so just replace the values as needed... cocor.indep.groups(r1, r2, n1, n2)}

\FunctionTok{cocor.indep.groups}\NormalTok{(}\FloatTok{0.2009678}\NormalTok{, }\FloatTok{0.6345603}\NormalTok{, }\DecValTok{73}\NormalTok{, }\DecValTok{77}\NormalTok{)}

\CommentTok{\# Final thing to do {-} graph these two correlations on the same plot to aid interpretation.}

\NormalTok{plot\_cc }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ positive\_SC, }\AttributeTok{y =}\NormalTok{ optimism, }\AttributeTok{colour =}\NormalTok{ extra\_curr)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{shape =}\NormalTok{ extra\_curr)) }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\FunctionTok{aes}\NormalTok{(}\AttributeTok{linetype =}\NormalTok{ extra\_curr), }\AttributeTok{method =} \StringTok{"lm"}\NormalTok{, }\AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Positive self compassion vs Optimism by Extra curricular activities"}\NormalTok{,}
       \AttributeTok{x =} \StringTok{"Positive self compassion"}\NormalTok{,}
       \AttributeTok{y =} \StringTok{"Optimism"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{scale\_color\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"0"} \OtherTok{=} \StringTok{"grey"}\NormalTok{, }\StringTok{"1"} \OtherTok{=} \StringTok{"black "}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{scale\_linetype\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"0"} \OtherTok{=} \StringTok{"solid"}\NormalTok{, }\StringTok{"1"} \OtherTok{=} \StringTok{"dashed"}\NormalTok{)) }\SpecialCharTok{+}
  \FunctionTok{scale\_shape\_manual}\NormalTok{(}\AttributeTok{values =} \FunctionTok{c}\NormalTok{(}\StringTok{"0"} \OtherTok{=} \DecValTok{16}\NormalTok{, }\StringTok{"1"} \OtherTok{=} \DecValTok{3}\NormalTok{))}

                     
\FunctionTok{print}\NormalTok{(plot\_cc)}

\CommentTok{\# WORKSHOP FINISHED {-} YAY!!!}
\end{Highlighting}
\end{Shaded}
...
\chapter{Workshop 17: Multiple Regression with Continuous Variables}\label{workshop-17-multiple-regression-with-continuous-variables}

\section{Exercise One}\label{exercise-one-1}

\textbf{Packages and Import Data}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Before doing any analyses, you need to get everything set up and ready...}
...
\FunctionTok{install.packages}\NormalTok{(correlation)}
\FunctionTok{install.packages}\NormalTok{(gridExtra)}

\FunctionTok{library}\NormalTok{(tidyverse)}
\FunctionTok{library}\NormalTok{(correlation)}
\FunctionTok{library}\NormalTok{(gridExtra)}

\CommentTok{\# Once you have done this, you should see them ticked in the "Packages" list {-} you can also manually tick the boxes.}
\CommentTok{\# If you need to install any of these, the code is install.packages("name of package")}
\CommentTok{\# Make sure you can then see it in the "Packages" list, and call it using library("name of package") or just tick it in the "Packages" list}
...
\section{Exercise Two}\label{exercise-two-2}
...
\CommentTok{\# You can also check the number of participants (obs) and the number of variables in the "Environment" tab.}
...
\CommentTok{\# We should run the zero order correlations for all continuous variables, the outcome variable and the predictors.}
...
\section{Exercise Three}\label{exercise-three-2}

\textbf{Run, Interpret, and Graph the Regression}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Now, let\textquotesingle{}s run a multiple regression.}
\CommentTok{\# We have SWL as the outcome variable that we want to predict}
\CommentTok{\# Then the three wellbeing measures and the number of negative life experiences giving us four continuous predictor variable.}

\NormalTok{model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(optimism }\SpecialCharTok{\textasciitilde{}}\NormalTok{ positive\_SC }\SpecialCharTok{+}\NormalTok{ negative\_SC }\SpecialCharTok{+}\NormalTok{ age\_years, }\AttributeTok{data =}\NormalTok{ mydata)}
\FunctionTok{summary}\NormalTok{(model)}

\CommentTok{\# We then want to create scatterplots to graphically represent any significant predictors (so three)}
...
\CommentTok{\# ncol = 3 tells R to put two next to each other, nrow = 1 tells R to put one above the other}
\end{Highlighting}
\end{Shaded}

\section{Exercise Four}\label{exercise-four-2}

\textbf{Hierarchical Regression}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Now, let\textquotesingle{}s move on to hierarchical regression {-} exactly what we just did, but adding years of education as a control variable.}

\CommentTok{\# First, let\textquotesingle{}s see if the control variable is significant by building model 1. Make sure it is called "model 1" and you need to run the summary to see the output.}

\NormalTok{model1 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(optimism }\SpecialCharTok{\textasciitilde{}}\NormalTok{ reading\_age, }\AttributeTok{data =}\NormalTok{ mydata)}
\FunctionTok{summary}\NormalTok{(model1)}

\CommentTok{\# Next, build our final model that has all the variables (control and predictor variables). This is "model 2", and again, use the summary to see the output.}

\NormalTok{model2 }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(optimism }\SpecialCharTok{\textasciitilde{}}\NormalTok{ reading\_age }\SpecialCharTok{+}\NormalTok{ positive\_SC }\SpecialCharTok{+}\NormalTok{ negative\_SC }\SpecialCharTok{+}\NormalTok{ age\_years, }\AttributeTok{data =}\NormalTok{ mydata)}
\FunctionTok{summary}\NormalTok{(model2)}

\CommentTok{\# Finally, we want to see if adding the predictor variables is significantly "better" than the control variable alone, this has two parts.}

\CommentTok{\# First {-} how much does the variance explained (adjusted R sq) increase?}

\NormalTok{r2\_control }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(model1)}\SpecialCharTok{$}\NormalTok{adj.r.squared  }\CommentTok{\# Adj Rsq of control model}
\NormalTok{r2\_full }\OtherTok{\textless{}{-}} \FunctionTok{summary}\NormalTok{(model2)}\SpecialCharTok{$}\NormalTok{adj.r.squared     }\CommentTok{\# Adj Rsq of full model}

\NormalTok{r2\_change }\OtherTok{\textless{}{-}}\NormalTok{ r2\_full }\SpecialCharTok{{-}}\NormalTok{ r2\_control  }

\FunctionTok{print}\NormalTok{(r2\_change)  }\CommentTok{\# Print the Adj Rsq change}

\CommentTok{\# Does the model significantly improve?}

\FunctionTok{anova}\NormalTok{(model1,model2)}

\CommentTok{\# We then want to create scatterplots to graphically represent any significant predictors (so two)}
...
\FunctionTok{grid.arrange}\NormalTok{(plot1, plot2, }\AttributeTok{nrow =} \DecValTok{1}\NormalTok{, }\AttributeTok{ncol =} \DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\chapter{\texorpdfstring{\textbf{Workshop 18: Multiple Regression with Binary and Interactive Variables}}{Workshop 18: Multiple Regression with Binary and Interactive Variables}}\label{workshop-18-multiple-regression-with-binary-and-interactive-variables}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{\textbf{Packages, Prepare, and Import Data}}{Packages, Prepare, and Import Data}}\label{packages-prepare-and-import-data}
...
\FunctionTok{install.packages}\NormalTok{(correlation)}
\FunctionTok{install.packages}\NormalTok{(gridExtra)}
\FunctionTok{install.packages}\NormalTok{(cocor)}
...
\NormalTok{mydata }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(optimism, positive\_SC, negative\_SC) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{correlation}\NormalTok{(}\AttributeTok{p\_adjust =} \StringTok{"none"}\NormalTok{)}
...
\section{\texorpdfstring{\textbf{Exercise One}}{Exercise One}}\label{exercise-one-2}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE ONE {-} running the multiple regression}

\CommentTok{\# Now, let\textquotesingle{}s run a multiple regression, but this time with continuous, binary and interactive predictors}
\CommentTok{\# We have SWL as the outcome variable that we want to predict}
\CommentTok{\# Then two continuous predictors (positive and negative SC), one binary (extra curricular) and two interactions (each SC measure interacting with extra curricular).}

\NormalTok{model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(optimism }\SpecialCharTok{\textasciitilde{}}\NormalTok{ positive\_SC }\SpecialCharTok{+}\NormalTok{ negative\_SC }\SpecialCharTok{+}\NormalTok{ extra\_curr }\SpecialCharTok{+}\NormalTok{ positive\_SC}\SpecialCharTok{*}\NormalTok{extra\_curr }\SpecialCharTok{+}\NormalTok{ negative\_SC}\SpecialCharTok{*}\NormalTok{extra\_curr, }\AttributeTok{data =}\NormalTok{ mydata)}
\FunctionTok{summary}\NormalTok{(model)}
\end{Highlighting}
\end{Shaded}

\section{\texorpdfstring{\textbf{Exercise Two}}{Exercise Two}}\label{exercise-two-3}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE TWO {-} interpreting the overall model}

\CommentTok{\# No additional bits to run for this, just pull out the relevant stats from the analysis you just ran.}
\end{Highlighting}
\end{Shaded}

\section{\texorpdfstring{\textbf{Exercise Three}}{Exercise Three}}\label{exercise-three-3}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE THREE {-} interpreting the continuous predictors and graph any significant predictors with a scatterplot}

\CommentTok{\# First, from the analysis already run, pull out the B, t and p for each continuous predictor and interpret significant findings.}

\CommentTok{\# Then create a scatterplot for any significant continuous predictors.}

\NormalTok{plot1 }\OtherTok{\textless{}{-}} \FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ negative\_SC, }\AttributeTok{y =}\NormalTok{ optimism)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{geom\_smooth}\NormalTok{(}\AttributeTok{method =} \StringTok{"lm"}\NormalTok{,}
              \AttributeTok{se =} \ConstantTok{FALSE}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}
...
\section{\texorpdfstring{\textbf{Exercise Four}}{Exercise Four}}\label{exercise-four-3}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE FOUR {-} interpreting the binary predictor, and if significant, create a graph and descriptives to aid interpretation}

\CommentTok{\# First, from the analysis already run, pull out the B, t and p for each continuous predictor and interpret significant findings.}

\CommentTok{\# Now, calculate the descriptive statistics (mean and SD) for each of the groups.}
...
\CommentTok{\# Next, we want to create a boxplot for each significant binary predictor. We do this in exactly the same way as for graphing an independent t test, so you can go back to that lecture/workshop if needed.}

\FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ extra\_curr, }\AttributeTok{y =}\NormalTok{ optimism)) }\SpecialCharTok{+}
  \FunctionTok{geom\_boxplot}\NormalTok{() }\SpecialCharTok{+}
  \FunctionTok{labs}\NormalTok{(}\AttributeTok{title =} \StringTok{"Optimism by doing extracurricular activities"}\NormalTok{,}
       \AttributeTok{x =} \StringTok{"Extracurricular activitiies"}\NormalTok{,}
       \AttributeTok{y =} \StringTok{"Mean optimism score"}\NormalTok{) }\SpecialCharTok{+}
  \FunctionTok{theme\_classic}\NormalTok{()}
\end{Highlighting}
\end{Shaded}

\section{\texorpdfstring{\textbf{Exercise Five}}{Exercise Five}}\label{exercise-five-2}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE FIVE {-} interpreting the interactive predictors}

\CommentTok{\# First, from the analysis already run, pull out the B, t and p for each interactive predictor and interpret significant findings.}

\CommentTok{\# For a significant interactive predictor, follow the four step process to statistically compare correlations.}

\CommentTok{\# STEP 1: We need to tell R which subgroups within our dataset we want to look at {-} so identify 0 and 1 from the "extra curricular" variable, and name each one.}

\NormalTok{None }\OtherTok{\textless{}{-}}\NormalTok{ mydata[mydata}\SpecialCharTok{$}\NormalTok{extra\_curr }\SpecialCharTok{==} \StringTok{"0"}\NormalTok{, ]}
\NormalTok{Activities }\OtherTok{\textless{}{-}}\NormalTok{ mydata[mydata}\SpecialCharTok{$}\NormalTok{extra\_curr }\SpecialCharTok{==} \StringTok{"1"}\NormalTok{, ]}

\CommentTok{\# STEP 2: Now we run the two correlations {-} we need to tell R first which subgroup to use from the naming we just did, and which continuous variable to correlate.}
...
\CommentTok{\# STEP 3: To compare the correlations statistically, we need the N and the r for each group. The r value is the final value in the output we just created {-} the final line, under corr.}
\CommentTok{\# To get the N for each group, we ran the "summary" earlier, but you can do it again to save scrolling. }
...
\CommentTok{\# STEP 4: graph these two correlations on the same plot to aid interpretation.}
...
\chapter{\texorpdfstring{\textbf{Workshop 19: Multiple Regression: Evaluating Models and Assumptions}}{Workshop 19: Multiple Regression: Evaluating Models and Assumptions}}\label{workshop-19-multiple-regression-evaluating-models-and-assumptions}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\section{\texorpdfstring{\textbf{Packages, Prepare, and Import Data}}{Packages, Prepare, and Import Data}}\label{packages-prepare-and-import-data-1}
...
\FunctionTok{install.packages}\NormalTok{(correlation)}
\FunctionTok{install.packages}\NormalTok{(gridExtra)}
\FunctionTok{install.packages}\NormalTok{(car)}
...
\section{\texorpdfstring{\textbf{Exercise One}}{Exercise One}}\label{exercise-one-3}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE ONE {-} running the multiple regression. We will not report it yet, but some of the assumptions need the "model" for its calculations.}

\NormalTok{model }\OtherTok{\textless{}{-}} \FunctionTok{lm}\NormalTok{(optimism }\SpecialCharTok{\textasciitilde{}}\NormalTok{ positive\_SC }\SpecialCharTok{+}\NormalTok{ negative\_SC }\SpecialCharTok{+}\NormalTok{ extra\_curr, }\AttributeTok{data =}\NormalTok{ mydata)}
\FunctionTok{summary}\NormalTok{(model)}
\end{Highlighting}
\end{Shaded}

\section{\texorpdfstring{\textbf{Exercise Two}}{Exercise Two}}\label{exercise-two-4}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE TWO {-} Looking at multicolinearity in three different ways}

\CommentTok{\# Multicolinearity, looking at r values across all predictor variabless.}

\NormalTok{mydata }\SpecialCharTok{\%\textgreater{}\%}
\NormalTok{  dplyr}\SpecialCharTok{::}\FunctionTok{select}\NormalTok{(positive\_SC, negative\_SC) }\SpecialCharTok{\%\textgreater{}\%}
  \FunctionTok{correlation}\NormalTok{(}\AttributeTok{p\_adjust =} \StringTok{"none"}\NormalTok{)}

\CommentTok{\# Multicolinearity, calculate VIF.}

\NormalTok{vif\_values }\OtherTok{\textless{}{-}} \FunctionTok{vif}\NormalTok{(model)}
\FunctionTok{print}\NormalTok{(vif\_values)}

\CommentTok{\# Multicolinearity, calculate tolerance. This is, essentially 1 {-} the R2 (so variance explained).}

\NormalTok{tolerance\_value }\OtherTok{\textless{}{-}} \DecValTok{1} \SpecialCharTok{{-}} \FunctionTok{summary}\NormalTok{(model)}\SpecialCharTok{$}\NormalTok{r.squared}
\FunctionTok{print}\NormalTok{(tolerance\_value)}
\end{Highlighting}
\end{Shaded}

\section{\texorpdfstring{\textbf{Exercise Three}}{Exercise Three}}\label{exercise-three-4}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE THREE {-} Normal distribution of residuals.}

\FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{residuals)) }\SpecialCharTok{+}
  \FunctionTok{geom\_histogram}\NormalTok{(}\AttributeTok{binwidth =} \FloatTok{0.5}\NormalTok{, }\AttributeTok{color =} \StringTok{"black"}\NormalTok{, }\AttributeTok{fill =} \StringTok{"white"}\NormalTok{) }\SpecialCharTok{+}
...
\section{\texorpdfstring{\textbf{Exercise Four}}{Exercise Four}}\label{exercise-four-4}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE FOUR {-} Homoscedasticity.}

\FunctionTok{ggplot}\NormalTok{(mydata, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{fitted.values, }\AttributeTok{y =}\NormalTok{ model}\SpecialCharTok{$}\NormalTok{residuals)) }\SpecialCharTok{+}
  \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+}
...
\section{\texorpdfstring{\textbf{Exercise Five}}{Exercise Five}}\label{exercise-five-3}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# EXERCISE FIVE{-}  Outliers.}

\CommentTok{\# Identify outliers using standardized residuals}
\NormalTok{standardized\_residuals }\OtherTok{\textless{}{-}} \FunctionTok{rstandard}\NormalTok{(model)}
\CommentTok{\# Print standardized residuals}
\FunctionTok{print}\NormalTok{(standardized\_residuals)}

\CommentTok{\# Determine the number of outliers (absolute value greater than 2)}
\NormalTok{outliers }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(}\FunctionTok{abs}\NormalTok{(standardized\_residuals) }\SpecialCharTok{\textgreater{}} \DecValTok{2}\NormalTok{)}
\FunctionTok{print}\NormalTok{(outliers)}

\CommentTok{\# Calculate the percentage of outliers}
\NormalTok{percentage\_outliers }\OtherTok{\textless{}{-}}\NormalTok{ (outliers }\SpecialCharTok{/} \FunctionTok{nrow}\NormalTok{(mydata)) }\SpecialCharTok{*} \DecValTok{100}
\FunctionTok{print}\NormalTok{(percentage\_outliers)}
\end{Highlighting}
\end{Shaded}
