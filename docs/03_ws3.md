...
Interpret your *t*-test. - Is it significant? - What is the effect size? - If significant, how do the groups differ?
...
- `before:after` will then take all and any columns from `before` through to `after` and re-arrange them into long data format.
...
desc <- longd %>%
  group_by(NULL) %>%
  summarise(mean_sleep = mean(NULL)
            sd_sleep = sd(NULL))
```
