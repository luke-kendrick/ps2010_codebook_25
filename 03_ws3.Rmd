...
We use a dollar sign `$` to point R to a particular column. For example, when using `shapiro.test(yes$total_sleep)` you are saying to run the Shapiro Test on the `yes` data set column called `total_score`
...
> **How do I ensure it is a Welch's *t*-test?** By setting `var.equal = FALSE` you are asking for a Welch's *t*-test. If you change this to `var.equal = TRUE` it will run a Studen't *t*-test, but if you do this, make sure you meet all the assumptions.
...
We will call the object `int` (short for intervention data)
...
(Hint: you will need to use the dollar sign `$` to specify which data set andcolumn, e.g., `data$column`).
...
-   Base it on the original data set called `int` and then `%>%`

-   `gather()`

-   The first argument is the name of your independent variable. I have called it `time` The second argument is the name of your dependent variable. I have called it `sleep`.
...
desc <- longd %>%
  group_by(NULL1) %>%
  summarise(mean_sleep = mean(NULL2)
            sd_sleep = sd(NULL2))
```

Change `NULL1` to the name of the independent variable in the `long` data file, and `NULL2` to the dependent variable. If this is tricky, use `names(long)` if you need a reminder of the variable names and revisit your notes from last week's workshop.
