...
Import the `energy.csv` data set. The code below assumes it will be called `mydata`.
...
scale_fill_manual(values = c("redcow" = "slategrey", "yumster" = "slategray1"), 
  ...
  scale_x_discrete(labels = c("morning" = "Morning", "afternoon" = "Afternoon"))
```
...
-   `labels =` will allow you to code what the text says within the legend itself. Here we can add capital letters. scale_x_discrete\`
...
Now for some real serious stuff...

`ggplot()` is really powerful and there are so many things you can do with it.

Copy and paste the code below, run it, and interpret the snowman plot...
