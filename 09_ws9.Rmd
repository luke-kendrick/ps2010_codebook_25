...
Import the data set called `exercise.csv`
...
``` r
ggplot(mydata_long, aes(x = time, y = exercise)) +
  geom_violin() +
...
wilcox_effsize(NULLL ~ NULL, data = longd, paired = TRUE)
```
...
## Code for Mann-Whitney U (or Wilcoxon Rank-Sum)
